"""三线程多频率 SpaceMouse 导纳遥操作 + LJL 夹爪控制 for RM75B.

合并自:
  - spacemouse_teleop_admittance_threaded.py  (三线程导纳: Input100Hz/Control200Hz/Record200Hz, 纯软件导纳 + movep_canfd)
  - spacemouse_teleop_switch.py (LJL)  (夹爪 incremental/binary/Switching/disabled + get_gripper_position 位置同步)

架构:
    SpaceMouseInputThread (100Hz) ──┬─► SharedState ─► ControlThread (200Hz) ─► rm_movep_canfd(follow=True)
                     └─ 夹爪控制 ──┘                   UDP 回调 (200Hz)
                                                      └─► RecordThread (200Hz)

夹爪相对原 threaded 版的改动(原版只支持 binary/Switching 且跳过了 incremental):
  * incremental 移到 InputThread 用 get_button hold 实现(按住持续开合);
  * setup 读真实夹爪位置(get_gripper_position)同步 SharedState.gripper_pos, 不再假设全开;
  * 新增 disabled 模式 + GRIPPER_OPEN_ON_EXIT 退出开关。
夹爪 Modbus/串口 与 运动 movep_canfd(TCP) 走独立通道, Modbus 写阻塞只影响 InputThread, 不拖累 200Hz 控制环。
"""
...
import config_admittance_threaded_gripper as cfg
...
class ThreadedAdmittanceTeleop(USBRelayController):
    """三线程导纳遥操作 + 夹爪控制主类。继承 USBRelayController 以便使用继电器夹爪。"""
    ...
    def setup(self):
        ...
        # 2. Arm (Switching/disabled 模式不启用夹爪 Modbus)
        ...
        # 7. 读真实夹爪位置同步基准(读不到则退化到假设全开)
        if cfg.GRIPPER_MODE not in ("Switching", "disabled"):
            pos = self.arm.get_gripper_position()
            if pos is not None:
                self.state.gripper_pos = float(pos)
                print(f"Gripper start pos: {int(round(self.state.gripper_pos))}")
            else:
                self.state.gripper_pos = float(cfg.GRIPPER_OPEN_POS)
                print("[WARN] Gripper position read failed, fallback to OPEN_POS")
        else:
            self.state.gripper_pos = float(cfg.GRIPPER_OPEN_POS)
    ...
    def start(self):
        ...
        print("Threads started: Input(100Hz) / Control(200Hz) / Record(200Hz)")
    ...
    # ------------------------------------------------------------------
    # Thread A: SpaceMouse input (100Hz) + 夹爪控制
    # ------------------------------------------------------------------

    def _spacemouse_input_loop(self):
        """读 SpaceMouse → 平滑 → scale → 积分 → 写 target_pose；并处理夹爪。"""
        ...
        while True:
            ...
            # 4. 写入 SharedState target_pose
            with self.state.lock_:
                self.state.target_pose = local_target.copy()

            # 5. 夹爪控制(消费按钮事件/hold, 在 InputThread 100Hz)
    ...
    def _handle_gripper(self):
        """夹爪控制，在 InputThread(100Hz) 调用。
        incremental 用 get_button hold(按住持续开合); binary/Switching 用事件; disabled 跳过。
        Modbus/串口写入与运动控制(独立 TCP 通道)互不干扰; set_gripper_position 内部限频 50ms。
        """
        mode = cfg.GRIPPER_MODE

        if mode == "disabled":
            self.mouse.pop_button_events()   # 仍消费，避免事件堆积
            return

        if mode == "incremental":
            # 消费事件队列以更新 hold 状态，再用 get_button 判断按住
            self.mouse.pop_button_events()
            with self.state.lock_:
                gp = self.state.gripper_pos
            if self.mouse.get_button(0):
                gp = max(cfg.GRIPPER_CLOSE_POS, gp - cfg.GRIPPER_INCREMENT)
                self.arm.set_gripper_position(gp)
                with self.state.lock_:
                    self.state.gripper_pos = gp
            elif self.mouse.get_button(1):
                gp = min(cfg.GRIPPER_OPEN_POS, gp + cfg.GRIPPER_INCREMENT)
                self.arm.set_gripper_position(gp)
                with self.state.lock_:
                    self.state.gripper_pos = gp
            ...
            for bnum, pressed in self.mouse.pop_button_events():
                ...
                if bnum == 0:
                    with self.state.lock_:
                        self.state.gripper_pos = float(cfg.GRIPPER_CLOSE_POS)
                    self.arm.set_gripper_position(cfg.GRIPPER_CLOSE_POS)
                    print("Gripper CLOSE")
                elif bnum == 1:
                    with self.state.lock_:
                        self.state.gripper_pos = float(cfg.GRIPPER_OPEN_POS)
    ...
    def _control_loop(self):
        ...
        while True:
            ...
            # 2. 读 target_pose（夹爪已移至 InputThread，这里不再读按钮事件）
            with self.state.lock_:
    ...
    # ------------------------------------------------------------------
    # Thread C: Record (200Hz)
    # ------------------------------------------------------------------

    def _record_loop(self):
        """读 SharedState 快照交给 _record_step。"""
    ...
    def _record_step(self, snapshot: dict):
        """每个周期的状态快照 → CSV 一行。RECORD_ENABLE=False 时是 no-op。"""
...
def main():
    parser = argparse.ArgumentParser(description="Threaded admittance teleop + gripper for RM75B")
