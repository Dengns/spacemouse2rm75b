"""SpaceMouse threaded admittance teleoperation config for RM75B — + LJL 夹爪合并版.

合并自:
  - config_admittance_threaded.py  (三线程频率 / UDP / 导纳 ADM_* / MOVEP_* / 录制, 全部保留)
  - config_switch.py (LJL)  (夹爪: incremental(默认) / binary / Switching / disabled + 退出开关)

与 config_admittance_threaded.py 的唯一差异: 夹爪段。
incremental 在 InputThread(100Hz) 用 get_button hold 实现(原 threaded 版跳过了 incremental)。
"""
...
# --- Gripper (合并 LJL: incremental 在 InputThread 用 hold 实现) ---
GRIPPER_OPEN_POS = 1000
GRIPPER_CLOSE_POS = 0
GRIPPER_MODE = "incremental"             # "binary"=按一下全开/全闭; "incremental"=按住持续开合;
                                         # "Switching"=USB继电器/电磁铁(左键吸芯片,右键放芯片);
                                         # "disabled"=不用手柄控夹爪, 保持原有状态
GRIPPER_INCREMENT = 50                   # incremental 每周期变化量 (InputThread 100Hz; 按住会快速到极值, 想慢就调小)
GRIPPER_OPEN_ON_EXIT = False             # True: 退出时自动张开; False: 保持当前位置
